...
import xyz.kd5ujc.schema.Updates.{ArchiveFiber, CreateStateMachineFiber, ProcessFiberEvent}
...
object FiberValidator {
  ...
  class L1Validator[F[_]: Monad](state: OnChain) {
    ...
    def createFiber(update: CreateStateMachineFiber): F[ValidationResult] =
      for {
    ...
    def processEvent(update: ProcessFiberEvent): F[ValidationResult] =
      for {
        cidExists      <- CommonRules.cidIsFound(update.cid, state)
        payloadNotNull <- CommonRules.isNotNull(update.event.payload, "event.payload")
        payloadSize <- CommonRules.valueWithinSizeLimit(
          update.event.payload,
          Limits.MaxEventPayloadBytes,
          "event.payload"
        )
        payloadStructure <- CommonRules.payloadStructureValid(update.event.payload, "event.payload")
      } yield List(cidExists, payloadNotNull, payloadSize, payloadStructure).combineAll

    /** Validates an ArchiveFiber update */
    def archiveFiber(update: ArchiveFiber): F[ValidationResult] =
  ...
  class L0Validator[F[_]: Async: SecurityProvider](
    ...
    def processEvent(update: ProcessFiberEvent): F[ValidationResult] =
      for {
        ...
        transitionOk  <- FiberRules.L0.transitionExists(update.cid, update.event.eventType, state.calculated)
